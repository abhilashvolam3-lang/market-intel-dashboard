...
const FEATURES = [
  ...
  { key: 'brand',     label: '🏷️ Brand',            desc: 'Brand landscape, top brands, challengers' },
  { key: 'sentiment', label: '💬 Sentiment',        desc: 'Customer reviews, top complaints, 5-star drivers' },
...
export default function Home() {
  ...
  const [filterFeature, setFilterFeature]       = useState<string>('overall')
  ...
  const generateSummary = async () => {
    ...
    try {
      ...
      const question = `Generate a focused ${featureLabel} analysis for ${companyLabel}, product type: ${productLabel}. Be specific with numbers, product names, prices, and actionable insights.`
    ...
    } catch { setSummary('Error generating summary.'); setSummaryGenerated(true) }
  ...
  const pill = (active: boolean, activeColor = '#3b82f6') => ({ padding: '7px 13px', borderRadius: 6, border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 600, transition: 'all .2s', background: active ? activeColor : 'transparent', color: active ? '#fff' : '#8892a4' } as React.CSSProperties)
    ...
    <main style={{ minHeight: '100vh', background: '#0f1117', fontFamily: "'DM Sans',system-ui,sans-serif", color: '#e8eaed' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg,#1a1f2e 0%,#0f1117 100%)', borderBottom: '1px solid #2a2f3e', padding: '18px 40px' }}>
        <div style={{ maxWidth: 1400, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
          <div>
            <h1 style={{ color: '#fff', fontSize: 22, fontWeight: 800, margin: 0, letterSpacing: '-0.5px' }}>🕯️ US Candle Market Intelligence</h1>
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '24px 40px' }}>
        ...
        {/* SOURCE CARDS — row 1: first 5 */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 10, marginBottom: 10 }}>
          {allSources.slice(0, 5).map(({ src, products }) => {
        ...
        {/* SOURCE CARDS — row 2: last 4 */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 22 }}>
          {allSources.slice(5).map(({ src, products }) => {
            const cfg = SOURCE_CONFIG[src]
            const avgP = avg(products.filter(p => p.price).map(p => p.price))
        ...
        <div style={{ display: 'flex', gap: 4, marginBottom: 22, background: '#1e2433', borderRadius: 12, padding: 4, width: 'fit-content' }}>
          {([{ key: 'analysis', label: '🧠 AI Analysis' }, { key: 'data', label: '📋 Data Explorer' }] as const).map(t => (
            <button key={t.key} onClick={() => setActiveTab(t.key)} style={{ padding: '10px 24px', borderRadius: 8, border: 'none', cursor: 'pointer', fontSize: 14, fontWeight: 600, transition: 'all .2s', background: activeTab === t.key ? '#3b82f6' : 'transparent', color: activeTab === t.key ? '#fff' : '#8892a4' }}>{t.label}</button>
          ))}
        </div>
          ...
          <div>
            ...
            <div style={{ ...card, padding: 28, marginBottom: 20 }}>
              ...
              <div style={{ marginBottom: 22 }}>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  ...
                  {Object.entries(SOURCE_DISPLAY_NAMES).map(([key, name]) => {
                    ...
                    return (
                      <button key={key} onClick={() => setFilterCompany(key as SourceKey)} style={{ padding: '8px 16px', borderRadius: 8, border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 600, transition: 'all .2s', background: active ? cfg.color : '#0f1117', color: active ? '#000' : '#8892a4', outline: active ? `1px solid ${cfg.color}` : '1px solid #2a2f3e' }}>
                        {cfg.icon} {name}
                      </button>
                    )
                  })}
              ...
              <div style={{ marginBottom: 22 }}>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  {PRODUCT_TYPES.map(pt => {
                    ...
                    return (
                      <button key={pt.key} onClick={() => setFilterProduct(pt.key as CandleType)} style={{ padding: '8px 16px', borderRadius: 8, border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 600, transition: 'all .2s', background: active ? '#8b5cf6' : '#0f1117', color: active ? '#fff' : '#8892a4', outline: active ? '1px solid #8b5cf6' : '1px solid #2a2f3e' }}>
                        {pt.label}
                      </button>
                    )
                  })}
              ...
              <div style={{ marginBottom: 24 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8 }}>
                  {FEATURES.map(f => {
                    ...
                    return (
                      <button key={f.key} onClick={() => setFilterFeature(f.key)} style={{ padding: '12px 16px', borderRadius: 8, border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 600, transition: 'all .2s', textAlign: 'left', background: active ? 'linear-gradient(135deg,#1e3a5f,#1e2433)' : '#0f1117', outline: active ? '1px solid #3b82f6' : '1px solid #2a2f3e', color: active ? '#fff' : '#8892a4' }}>
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 20, alignItems: 'center' }}>
                ...
                <span style={{ background: '#3b82f620', color: '#60a5fa', fontSize: 11, padding: '3px 10px', borderRadius: 20, fontWeight: 600 }}>
                  {filterCompany === 'all' ? '🌐 All Companies' : `${SOURCE_CONFIG[filterCompany]?.icon} ${SOURCE_DISPLAY_NAMES[filterCompany]}`}
                </span>
                <span style={{ background: '#8b5cf620', color: '#a78bfa', fontSize: 11, padding: '3px 10px', borderRadius: 20, fontWeight: 600 }}>
                  {PRODUCT_TYPES.find(p => p.key === filterProduct)?.label}
                </span>
                <span style={{ background: '#10b98120', color: '#34d399', fontSize: 11, padding: '3px 10px', borderRadius: 20, fontWeight: 600 }}>
                  {FEATURES.find(f => f.key === filterFeature)?.label}
                </span>
              </div>
            ...
            <div style={{ ...card, padding: 20 }}>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                <button onClick={askQuestion} disabled={chatLoading || !chatQuestion.trim()} style={{ background: chatLoading ? '#1e2433' : 'linear-gradient(135deg,#10b981,#3b82f6)', color: '#fff', border: 'none', borderRadius: 8, padding: '11px 20px', fontSize: 13, fontWeight: 700, cursor: chatLoading ? 'not-allowed' : 'pointer', opacity: chatLoading ? 0.6 : 1, whiteSpace: 'nowrap' }}>{chatLoading ? '⏳' : '✨ Ask'}</button>
          ...
          <div>
            <div style={{ display: 'flex', gap: 8, marginBottom: 14, flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: 3, background: '#1e2433', borderRadius: 8, padding: 3, flexWrap: 'wrap' }}>
                {([
                  { key: 'all', label: '🌐 All' }, { key: 'amazon', label: '📦 Amazon' }, { key: 'pfcandleco', label: '✨ PF' },
                  { key: 'homesick', label: '🏠 Homesick' }, { key: 'paddywax', label: '🕯️ Paddywax' }, { key: 'otherland', label: '🌿 Otherland' },
                  { key: 'boysmells', label: '🌸 Boy Smells' }, { key: 'keap', label: '🌱 Keap' }, { key: 'asda', label: '🛒 ASDA' }, { key: 'primark', label: '👜 Primark' },
                ] as const).map(s => (
                  <button key={s.key} onClick={() => setActiveSource(s.key)} style={pill(activeSource === s.key,
                    s.key === 'amazon' ? '#ff9900' : s.key === 'pfcandleco' ? '#8b5cf6' : s.key === 'homesick' ? '#f97316' :
                    s.key === 'paddywax' ? '#10b981' : s.key === 'otherland' ? '#ec4899' : s.key === 'boysmells' ? '#8b5cf6' :
                    s.key === 'keap' ? '#34d399' : s.key === 'asda' ? '#84cc16' : s.key === 'primark' ? '#f9a8d4' : '#3b82f6'
                  )}>{s.label}</button>
                ))}
              </div>
              <div style={{ display: 'flex', gap: 3, background: '#1e2433', borderRadius: 8, padding: 3 }}>
                {([{ key: 'all', label: 'All Types' }, { key: 'jar-container', label: '🫙 Jar' }, { key: 'multi-pack', label: '🎁 Pack' }, { key: 'tea-light', label: '🕯️ Tea' }, { key: 'taper-pillar', label: '🕍 Taper' }] as const).map(t => (
              ...
              {filteredProducts.map(p => {
                ...
                return (
                  <div key={p.id} onClick={() => setSelectedProduct(isSelected ? null : p)}
                    style={{ ...card, padding: 14, border: isSelected ? '1px solid #3b82f6' : '1px solid #2a2f3e', background: isSelected ? 'linear-gradient(135deg,#1e3a5f 0%,#1e2433 100%)' : card.background, cursor: 'pointer', transition: 'all .2s' }}
                    ...
                    {isSelected && (
                      <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid #2a2f3e' }}>
                        ...
                        <div style={{ display: 'flex', gap: 7, marginTop: 9, flexWrap: 'wrap' }}>
                          ...
                          {p.source === 'pfcandleco' && <a href="https://pfcandleco.com/collections/all" target="_blank" rel="noopener noreferrer" style={{ background: '#8b5cf6', color: '#fff', fontSize: 11, fontWeight: 700, padding: '5px 12px', borderRadius: 6, textDecoration: 'none' }}>PF Candle Co ↗</a>}
