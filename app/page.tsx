...
export default function Home() {
  ...
  const generateSummary = async () => {
    ...
    try {
      ...
      const question = `Generate a focused ${featureLabel} analysis for ${companyLabel}, product type: ${productLabel}. Be specific with numbers and actionable insights.`
  ...
  const filteredCount = filterCompany !== 'all'
    ...
    : filterProduct !== 'all'
      ? allProducts.filter(p => p.candle_type === filterProduct).length
      : allProducts.length
      ...
      <div style={{ background:'linear-gradient(135deg,#1a1f2e 0%,#0f1117 100%)', borderBottom:'1px solid #2a2f3e', padding:'18px 40px' }}>
        <div style={{ maxWidth:1400, margin:'0 auto', display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:12 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:12, flexWrap:'wrap' }}>
            ...
            {runStatus && <span style={{ fontSize:12, color:'#10b981' }}>{runStatus}</span>}
      ...
      <div style={{ maxWidth:1400, margin:'0 auto', padding:'24px 40px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(7,1fr)', gap:10, marginBottom:16 }}>
          {[
            ...
            { label:'Avg Rating',       value:avg(validStars).toFixed(1)+' stars', sub:'Combined sources', color:'#f59e0b' },
            ...
            <div key={i} style={{ ...card, padding:'14px 12px', borderTop:`3px solid ${m.color}` }}
        ...
        {/* SOURCE CARDS — compact single row of 9 */}
          ...
          {srcList.map(({src,products}) => {
            ...
            return (
              <div key={src} onClick={()=>{setActiveTab('data');setActiveSource(src as SourceKey)}}
                ...
                <p style={{ color:'#8892a4', fontSize:9, fontWeight:700, textTransform:'uppercase', margin:'0 0 2px', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{cfg.icon} {SOURCE_DISPLAY_NAMES[src]}</p>
          ...
          <div>
            <div style={{ ...card, padding:28, marginBottom:20 }}>
              ...
              <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:20, alignItems:'center' }}>
                ...
                <span style={{ background:'#3b82f610', color:'#8892a4', fontSize:11, padding:'3px 10px', borderRadius:20 }}>{filteredCount} products</span>
              </div>
                ...
                {summaryLoading ? (
                  <div style={{ textAlign:'center', padding:'40px 0' }}>
                    ...
                    <p style={{ color:'#8892a4', fontSize:14 }}>
                      Analysing {filteredCount} {filterCompany!=='all'?SOURCE_DISPLAY_NAMES[filterCompany]+' ':''}{filterProduct!=='all'?filterProduct+' ':''}products...
                    </p>
                  </div>
                ) : (
                  <>
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20, flexWrap:'wrap', gap:8 }}>
                      <div>
                        <h3 style={{ color:'#fff', fontSize:16, fontWeight:700, margin:0 }}>{FEATURES.find(f=>f.key===filterFeature)?.label}</h3>
                        <p style={{ color:'#8892a4', fontSize:12, margin:'4px 0 0' }}>{filterCompany==='all'?'All Companies':SOURCE_DISPLAY_NAMES[filterCompany]} · {PRODUCT_TYPES.find(p=>p.key===filterProduct)?.label} · {filteredCount} products</p>
                      </div>
            ...
            <div style={{ ...card, padding:20 }}>
              ...
              <div style={{ display:'flex', gap:10 }}>
                ...
                <button onClick={askQuestion} disabled={chatLoading||!chatQuestion.trim()} style={{ background:chatLoading?'#1e2433':'linear-gradient(135deg,#10b981,#3b82f6)', color:'#fff', border:'none', borderRadius:8, padding:'11px 20px', fontSize:13, fontWeight:700, cursor:chatLoading?'not-allowed':'pointer', opacity:chatLoading?0.6:1 }}>{chatLoading?'Thinking...':'Ask'}</button>
          ...
          <div>
            <div style={{ display:'flex', gap:8, marginBottom:14, flexWrap:'wrap', alignItems:'center' }}>
              ...
              <div style={{ display:'flex', gap:3, background:'#1e2433', borderRadius:8, padding:3 }}>
                {([{key:'all',label:'All'},{key:'jar-container',label:'Jar'},{key:'multi-pack',label:'Pack'},{key:'tea-light',label:'Tea'},{key:'taper-pillar',label:'Taper'}] as const).map(t=>(
                    ...
                    <div style={{ display:'flex', gap:10, alignItems:'flex-start' }}>
                      ...
                      <div style={{ flex:1, minWidth:0 }}>
                        <div style={{ display:'flex', gap:4, marginBottom:4, flexWrap:'wrap' }}>
                          ...
                          {p.candle_type&&<span style={{ fontSize:10,fontWeight:700,padding:'2px 6px',borderRadius:4,background:'#8b5cf620',color:'#a78bfa' }}>{p.candle_type==='jar-container'?'Jar':p.candle_type==='multi-pack'?'Pack':p.candle_type==='tea-light'?'Tea':p.candle_type==='taper-pillar'?'Taper':p.candle_type}</span>}
                          {p.is_scented===true&&<span style={{ fontSize:10,fontWeight:700,padding:'2px 6px',borderRadius:4,background:'#10b98120',color:'#34d399' }}>Scented</span>}
                        ...
                        <div style={{ display:'flex',gap:6,flexWrap:'wrap',alignItems:'center' }}>
                          ...
                          {p.stars&&<span style={{ color:'#fbbf24',fontSize:11 }}>{p.stars} stars</span>}
