...
export default function Home() {
  ...
  const [sortBy, setSortBy] = useState<'reviews_count' | 'stars' | 'price' | 'burn_hours' | 'burn_per_oz'>('reviews_count')
      ...
      <div style={{ maxWidth: '1300px', margin: '0 auto', padding: '28px 40px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: '14px', marginBottom: '28px' }}>
          {[
            ...
            >
              <p style={{ color: '#8892a4', fontSize: '10px', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.8px', margin: '0 0 8px' }}>{m.label}</p>
              <p style={{ color: '#fff', fontSize: '20px', fontWeight: '800', margin: '0 0 4px' }}>{m.value}</p>
              <p style={{ color: '#8892a4', fontSize: '10px', margin: 0 }}>{m.sub}</p>
            </div>
          ))}
        </div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: '20px' }}>
              <button onClick={() => setAnalysisFilter('all')} style={{
                ...
                background: analysisFilter === 'all' ? '#3b82f6' : '#1e2433', color: analysisFilter === 'all' ? '#fff' : '#8892a4'
              }}>All Sections</button>
              {sections.map(s => (
            ...
            <div style={{ display: 'flex', gap: 12, marginBottom: '20px', flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              <input
                ...
                style={{
                  ...
                  padding: '10px 16px', color: '#fff', fontSize: '13px', width: '260px',
                  outline: 'none'
                }}
              />
              ...
              <div style={{ display: 'flex', gap: 4, background: '#1e2433', borderRadius: '8px', padding: '4px' }}>
                {(['all', 'amazon', 'walmart'] as const).map(s => (
                  <button key={s} onClick={() => setActiveSource(s)} style={{
                    padding: '7px 16px', borderRadius: '6px', border: 'none', cursor: 'pointer', fontSize: '12px', fontWeight: '600',
                    background: activeSource === s ? (s === 'amazon' ? '#ff9900' : s === 'walmart' ? '#0071ce' : '#3b82f6') : 'transparent',
                    color: activeSource === s ? '#fff' : '#8892a4',
                    transition: 'all 0.2s'
              ...
              <select
                ...
                style={{
                  background: '#1e2433', border: '1px solid #2a2f3e', borderRadius: '8px',
                  padding: '10px 16px', color: '#fff', fontSize: '13px', cursor: 'pointer', outline: 'none'
                }}
              >
                  ...
                  <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', gap: 6, marginBottom: 4 }}>
                        ...
                        {p.burn_hours && <span style={{ fontSize: '10px', fontWeight: '700', padding: '2px 8px', borderRadius: '4px', background: '#10b98120', color: '#34d399' }}>🔥 {p.burn_hours}hrs</span>}
                      </div>
                    ...
                    <div style={{ marginTop: 14, paddingTop: 14, borderTop: '1px solid #2a2f3e' }}>
                      ...
                      {p.asin && p.source === 'amazon' && (
                        <a href={`https://www.amazon.com/dp/${p.asin}`} target="_blank" rel="noopener noreferrer"
                          style={{ display: 'inline-block', marginTop: 10, background: '#ff9900', color: '#000', fontSize: '11px', fontWeight: '700', padding: '6px 14px', borderRadius: '6px', textDecoration: 'none' }}>
                          View on Amazon ↗
                        </a>
                      )}
                    </div>
